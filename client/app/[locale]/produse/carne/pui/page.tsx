...
export default function ChickenProductsPage() {
  ...
  useEffect(() => {
    fetch(`http://localhost:8000/api/products?category=Pui`)
      .then((res) => res.json())
      .then(setProducts);
  }, []);
  ...
  const subtitle =
    locale === "ro"
      ? "Produse de calitate superioară din carne de pui."
      : "Premium quality chicken products.";
        ...
        <div className="backdrop-blur-md bg-black/70 border border-yellow-500/20 rounded-3xl shadow-2xl p-8 sm:p-12">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {products.map((product) => (
              <Link
                key={product.name}
                href={`/${locale}/produse/carne/pui/${encodeURIComponent(
                  product.name
                )}`}
